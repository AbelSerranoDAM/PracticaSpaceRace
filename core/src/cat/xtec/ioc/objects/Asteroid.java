...
public class Asteroid extends Scrollable {
    ...
    public boolean collidesStation(Estacion station) {

        if (position.x <= station.getX() + station.getWidth()) {
            // Comprovem si han col·lisionat sempre i quan la estacio estigui a la mateixa alçada que la spacecraft
            if (Intersector.overlaps(collisionCircle, station.getCollisionCircle())) {
                return true;
            }
        }
        return false;
